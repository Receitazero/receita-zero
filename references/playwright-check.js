...
(async () => {
  ...
  const pages = [
    ...
    { url: 'http://127.0.0.1:8736/site-dfy/pet/premium/index.html', name: 'pet premium (Vitrine Certa)' },
  ];
  ...
  for (const page of pages) {
    try {
      ...
      // reveal ativado usa '.in' (landing legada) ou '.on' (paralelo/refino premium) — aceitar ambos
      const hidden = await p.$$eval('.reveal:not(.on):not(.in)', el => el.length).catch(() => 0);
